...
export const init = (config: InitConfig): Model => ({
  popover: FoldkitPopover.init({ id: config.id, isAnimated: config.isAnimated ?? true, contentFocus: true }),
...
const foldPopover = Update.foldChild({
  ...
  toParentOutMessage: (
    outMessage: FoldkitPopover.OutMessage,
  ): Option.Option<OutMessage> =>
