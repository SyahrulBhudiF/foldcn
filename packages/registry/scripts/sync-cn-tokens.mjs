 ...
 *   5. With --inject: merges cn-tokens.css + cn-compat.css into the style
 *      item's `css` object (registry/default/style/registry.json) under
 *      "@layer components", so the shadcn CLI writes them into the user's
 *      globals.css on install.
 *
 ...
 *
 * Usage:
 *   node scripts/sync-cn-tokens.mjs [--shadcn-dir <path>] [--inject]
 *
...
const COMPAT_FILE = join(STYLE_DIR, 'cn-compat.css')
const MANIFEST_FILE = join(STYLE_DIR, 'registry.json')
 ...
 *
 * Hand-maintained foldkit deltas live in cn-compat.css (imported after this
 * file), e.g. aria-disabled/data-disabled twins for controls where foldkit
 * cannot emit the native disabled attribute.
 *
...
// --- inject into the style manifest ----------------------------------------

if (shouldInject) {
  const manifest = JSON.parse(readFileSync(MANIFEST_FILE, 'utf8'))
  const item = manifest.items.find((entry) => entry.type === 'registry:style')
  if (!item) {
    console.error(`no registry:style item in ${MANIFEST_FILE}`)
    process.exit(1)
  }

  const layerRules = {}
  const topLevels = {}
  for (const [token, rule] of definedTokens) {
    if (!referencedTokens.has(token)) continue
    layerRules[`.${token}`] = { [`@apply ${rule.transformed}`]: {} }
  }
  for (const rule of parseCompat(COMPAT_FILE)) {
    if (rule.selector.startsWith('@utility')) {
      // @utility must be top-level, not nested in a layer
      topLevels[rule.selector] = cssTextToObject(rule.raw)
    } else {
      layerRules[rule.selector] = rule.apply ? { [`@apply ${rule.apply}`]: {} } : {}
    }
  }

  item.css = { ...item.css, ...topLevels, '@layer components': layerRules }
  writeFileSync(MANIFEST_FILE, JSON.stringify(manifest, null, 2) + '\n')
  console.log(
    `registry.json: injected ${Object.keys(layerRules).length} component-layer rules into ${item.name} style item`,
  )
}
...
/** Convert flat/nested css declaration text into the manifest's object shape
 *  ("prop": "value", "&child": { ... }). Supports one nesting level, which is
 *  all the compat utilities need. */
function cssTextToObject(text) {
  const out = {}
  const re = /([&a-zA-Z-]+)\s*:\s*([^;{}]+);|([&][\w:.-]+|\.[\w-]+)\s*\{([^{}]*)\}/g
  for (const m of text.matchAll(re)) {
    if (m[1]) out[m[1]] = m[2].trim()
    else if (m[3]) {
      out[m[3]] = {}
      for (const p of m[4].matchAll(/([a-zA-Z-]+)\s*:\s*([^;]+);/g)) {
        out[m[3]][p[1]] = p[2].trim()
      }
    }
  }
  return out
...
/** Parse hand-written compat css: `.sel { @apply x y; }`, `.sel { }`,
 *  and verbatim top-level at-rules like `@utility name { ... }`. */
function parseCompat(path) {
  if (!existsSync(path)) return []
  const src = readFileSync(path, 'utf8')
  const rules = []
  // strip comments so braces inside them never confuse matching
  const clean = src.replace(/\/\*[\s\S]*?\*\//g, '')
  for (const match of clean.matchAll(/\.([a-zA-Z0-9_-]+)\s*\{([^}]*)\}/g)) {
    const body = match[2]
    const applyMatch = body.match(/@apply\s+([^;}]+)/)
    rules.push({
      selector: `.${match[1]}`,
      apply: applyMatch ? applyMatch[1].trim().replace(/\s+/g, ' ') : '',
    })
  }
  for (const match of clean.matchAll(/@(utility [a-zA-Z0-9_-]+)\s*\{((?:[^{}]*|\{[^{}]*\})*)\}/g)) {
    rules.push({ selector: `@${match[1]}`, apply: null, raw: match[2].trim() })
  }
  return rules
}
