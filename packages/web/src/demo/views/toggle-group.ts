...
const staticGroup = (
  ...
  const variant = (opts?.variant ?? 'default') as toggle.ToggleVariant
  const size = (opts?.size ?? 'default') as toggle.ToggleSize
    ...
    items.map((item) =>
      h.button(
        [
          ...
          h.AriaPressed('false'),
          h.Class([toggle.toggleBase, toggle.toggleVariants[variant], toggle.toggleSizes[size], toggleGroup.toggleGroupItemClass].join(' ')),
        ],
        [item.icon === undefined ? item.label : h.span([], [icon(h, item.icon, 'size-4'), ` ${item.label}`.trimStart()])],
      ),
    ),
  )
}
    ...
    [
      ...
      h.div(
        ...
        [
          ...
          staticGroup(h, [{ value: 'all', label: 'All' }, { value: 'missed', label: 'Missed' }], { variant: 'outline' }),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          ...
          h.div(
            ...
            [
              staticGroup(h, [{ value: 'top', label: 'Top' }, { value: 'bottom', label: 'Bottom' }, { value: 'left', label: 'Left' }, { value: 'right', label: 'Right' }], { variant: 'outline', size: 'sm' }),
              staticGroup(h, [{ value: 'top', label: 'Top' }, { value: 'bottom', label: 'Bottom' }, { value: 'left', label: 'Left' }, { value: 'right', label: 'Right' }], { variant: 'outline', size: 'default' }),
            ],
          ),
        ],
      ),
      h.div(
        ...
        [
          ...
          staticGroup(h, [{ value: 'star', label: 'Star', icon: Star }, { value: 'heart', label: 'Heart', icon: Heart }, { value: 'bookmark', label: 'Bookmark', icon: Bookmark }], { variant: 'outline', spacing: 2, size: 'sm' }),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          ...
          staticGroup(h, [{ value: 'all', label: 'All' }, { value: 'active', label: 'Active' }, { value: 'completed', label: 'Completed' }, { value: 'archived', label: 'Archived' }], { variant: 'outline', size: 'sm' }),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          ...
          staticGroup(h, [{ value: 'newest', label: 'Newest' }, { value: 'oldest', label: 'Oldest' }, { value: 'popular', label: 'Popular' }], { variant: 'outline', size: 'sm' }),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          ...
          staticGroup(h, [{ value: 'bold', label: '', icon: Bold }, { value: 'italic', label: '', icon: Italic }, { value: 'underline', label: '', icon: Underline }], { orientation: 'vertical', spacing: 1 }),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          h.div([h.Class('px-1 text-xs font-medium text-muted-foreground')], ['Font Weight Selector']),
          field<AppMessage>(
            {},
            [
