...
import { chevronDownIcon, chevronLeftIcon, chevronRightIcon, checkIcon, minusIcon, xIcon } from '@foldcn/registry/src/lib/icons'
...
export const inputView = (model: Model, h: HtmlBuilder<Message>): Html =>
  h.div([h.Class('flex w-full max-w-sm flex-col gap-4')], [
    input<Message>({
      id: 'input-email',
      label: 'Email',
      type: 'email',
...
export const checkboxView = (model: Model, h: HtmlBuilder<Message>): Html =>
  h.div([h.Class('flex w-full max-w-sm flex-col gap-5')], [
    checkbox<Message>({
      id: 'checkbox-terms',
...
const ICON_ROWS: ReadonlyArray<ReadonlyArray<[string, (h: HtmlBuilder<Message>) => Html]>> = [
  [['check', h => checkIcon(h)], ['chevron-down', h => chevronDownIcon(h)], ['chevron-left', h => chevronLeftIcon(h)], ['chevron-right', h => chevronRightIcon(h)], ['minus', h => minusIcon(h)], ['x', h => xIcon(h)]],
]

export const iconsView = (model: Model, h: HtmlBuilder<Message>): Html =>
  h.div([h.Class('w-full max-w-lg')], [
    h.p(
    ...
    ...ICON_ROWS.map(row =>
      h.div([h.Class('mb-2 grid grid-cols-6 gap-2')], row.map(([label, render]) =>
        h.div([h.Class('flex flex-col items-center gap-1 rounded-lg border border-border bg-card p-3 text-muted-foreground')], [
          render(h),
          h.span([h.Class('text-[11px]')], [label]),
        ]),
      )),
    ),
  ])