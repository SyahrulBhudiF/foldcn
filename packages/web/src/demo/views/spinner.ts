    ...
    [
      h.div(
        ...
        [
          ...
          h.div([h.Class('flex items-center gap-6')], [spinner<Message>({}, h), spinner<Message>({ className: 'size-6' }, h)]),
        ],
      ),
      h.div(
        [h.Class('flex w-full flex-col gap-2')],
        [
          ...
          h.div(
            ...
            [
              ...
              button<Message>({ isDisabled: true }, h.span([], [spinner<Message>({}, h), ' Disabled']), h),
              button<Message>({ variant: 'outline', isDisabled: true }, h.span([], [spinner<Message>({}, h), ' Outline']), h),
              button<Message>({ variant: 'outline', size: 'icon', isDisabled: true, attributes: [h.AriaLabel('Loading')] }, spinner<Message>({}, h), h),
            ],
          ),
        ],
      ),
      ...
      h.div(
        ...
        [
          ...
          h.div(
            ...
            [
              h.label([h.Class('text-sm font-medium'), h.For('input-group-spinner')], ['Input Group']),
              h.div(
                [h.Class('flex items-center gap-2 rounded-lg border px-2.5 py-1')],
                [h.input([h.Id('input-group-spinner'), h.Class('flex-1 bg-transparent outline-none text-sm'), h.Placeholder('Search...')]), spinner<Message>({}, h)],
              ),
            ],
          ),
        ],
      ...
      h.div(
        ...
        [
          ...
          h.div(
            [h.Class('flex min-h-[280px] w-full flex-col items-center justify-center gap-4 rounded-lg border py-8')],
            [
              spinner<Message>({}, h),
              h.div([h.Class('text-sm font-medium')], ['No projects yet']),
              h.div([h.Class('text-sm text-muted-foreground text-center max-w-xs')], ['You haven\'t created any projects yet. Get started by creating your first project.']),
              h.div(
                [h.Class('flex gap-2')],
                [button<Message>({}, 'Create project', h), button<Message>({ variant: 'outline' }, 'Import project', h)],
              ),
              h.a(
                [h.Href('#'), h.Class('text-sm text-muted-foreground underline-offset-4 hover:underline inline-flex items-center gap-1')],
