...
shadcn/ui checkout (upstream)
  ...
  styles/style-nova.css             ← .cn-* token definitions
        │
        │  packages/registry/scripts/sync-cn-tokens.mjs --inject
        ▼
...
registry/default/style/registry.json    ← tokens injected into css["@layer components"]
        │
        ├── web/src/styles.css imports both css files (showcase)
        └── shadcn build → dist/r/foldcn.json (users get them in globals.css)
```

**Component files emit only token classes** (`cn-button cn-button-variant-default …`),
so a component's class strings stay diffable against upstream and visual changes are
one CSS edit away. Never inline resolved utilities in component files anymore.
...
6. Run `pnpm --filter @foldcn/web build && pnpm --filter @foldcn/web test`,
   then check the demo page visually.
...
Hand-maintained, merged into the style manifest by `--inject`, imported after
`cn-tokens.css` everywhere. Put a rule here when:
...
node packages/registry/scripts/sync-cn-tokens.mjs --inject
pnpm --filter @foldcn/registry build   # shadcn build → dist/r
...
they exist as extension points. foldcn keeps them as inert classes for
derivation fidelity: `cn-tabs-list-variant-{default,line}`, `cn-progress-root`,
`cn-resizable-{panel-group,handle}`, `cn-marker-variant-default`,
`cn-sidebar-trigger`, `cn-calendar-day-button`, `cn-pagination*` and others —
see the header of `cn-tokens.css` for the authoritative list.
